...
public class Graphe {
	...
	private static int somme_tab_tmp=0;
		...
		public Graphe() {
		  ...
		  public static void main(String[] args) throws IOException {
			  ...
			  try {
					...
					if (ligne.contains("c [graph]"))
					{
						...
						while(j<nbr_arc_total) {
							...
							while(u<actual_node) {
								...
								if(tab_sommet[u].test_chemin(Integer.parseInt(l[0]), Integer.parseInt(l[1]))) {
									if(!noeud_existe(Integer.parseInt(l[1]))) {
										
										add_sommet(Integer.parseInt(l[1]), tab_sommet[u].get_k_from(Integer.parseInt(l[1])) ,tab_sommet[u].get_tab_route_from(Integer.parseInt(l[1])));
									...
									if(!noeud_existe(Integer.parseInt(l[0]))) {
										...
										add_sommet(Integer.parseInt(l[0]), tab_sommet[u].get_k_from(Integer.parseInt(l[0])) ,tab_sommet[u].get_tab_route_from(Integer.parseInt(l[0])));
				...
				while ((ligne_solution=br_solution.readLine())!=null){
					if (ligne_solution.contains("exemple_graphe"))
					{
						...
						while(o<nbr_node_evac_solution) {		
							...
							int v=0;
							while(v<tab_sommet.length) {
								
								if(tab_sommet[v].test_id(id_solution_tmp)){
									int w=0;
									int population=tab_sommet[v].get_population();
									int nbr_chemin=tab_sommet[v].get_k();
									int tab_chemin[];
									tab_chemin=new int[nbr_chemin];
									tab_chemin=tab_sommet[v].get_next_node_tab();
									//remplier tab_chemin avec le retour de tab sommet : tab_sommet[v].get_next_node_tab();
									while(w<nbr_chemin){
										int next_node_tmp=tab_chemin[w+1];
										int u=0;
										while(u<tab_arc.length) {
											if(tab_arc[u].test_arc(id_solution_tmp,next_node_tmp)){
												tab_arc[u].add_tab_evenement(date_solution_tmp,(int) Math.ceil((float)population/taux_solution_tmp),taux_solution_tmp,(population-(((int) Math.ceil((float)population/taux_solution_tmp)-1)*taux_solution_tmp)));
												date_solution_tmp=date_solution_tmp+tab_arc[u].get_length();
												u=tab_arc.length;
											}
											u++;
										}
				...
				int z=0;
				while(z<tab_arc.length) {
				//	tab_arc[z].print_tab_evenement();
					if(tab_arc[z].verif_tab_evenement()!=true) {
		...
		public static void add_arc(int id1, int id2, double duedate, int length, int capacity) {
			tab_arc[actual_arc]= new Arc(id1,id2,duedate,length,capacity);	
			actual_arc++;
		...
		public static void add_sommet(int id,int pop,int rate, int nbr_node, int[] tab_escape){
			...
			tab_sommet[actual_node]=new Sommet(id, pop, rate, nbr_node, tab_escape);
			actual_node++;
		...
		public static void add_sommet(int id, int nbr_node, int[] tab_escape) {
			...
			tab_sommet[actual_node]=new Sommet(id, nbr_node, tab_escape);
			actual_node++;
