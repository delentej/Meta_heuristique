...
public class Arc {
	...
	public void add_tab_evenement(int index,int nbr_envoie,int nbr_personne_par_envoie, int nbr_dernier_envoie) {
		...
		*/
		...
		while(j<nbr_envoie) {
			if(j==(nbr_envoie-1)) {
				tab_evenement[index]=tab_evenement[index]+nbr_dernier_envoie;
			}
			else {
				tab_evenement[index]=tab_evenement[index]+nbr_personne_par_envoie;
			}
			j++;
			index++;
		}
	...
	public void print_tab_evenement() {
		...
		int p=0;
		while(p<tab_evenement.length) {
			System.out.println(p + " : " + tab_evenement[p]);
			p++;
		}
	...
	public boolean verif_tab_evenement() {
		...
		int i=0;
		while(i<tab_evenement.length) {
			
			if(tab_evenement[i]>capacity) {
